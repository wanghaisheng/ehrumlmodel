...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="pocResultSectionCCDA">
        <xsl:param name="pocResultSection"/>
		<Service>
			<xsl:for-each select="$pocResultSection/@moodCode"> 
				<ServiceType> 
					<xsl:sequence select="fn:string(.)"/>
				</ServiceType> 
			</xsl:for-each>
						
                <xsl:for-each select="$pocResultSection/cda:id">
                ...
                <xsl:for-each select="$pocResultSection/cda:code">

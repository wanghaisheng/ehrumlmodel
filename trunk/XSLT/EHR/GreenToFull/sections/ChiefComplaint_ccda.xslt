...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="chiefComplaintCCDA">
        <xsl:param name="chiefComplaint" />
		
		<xsl:for-each select="$chiefComplaint">
			<component>
				<section>
					<templateId root="1.3.6.1.4.1.19376.1.5.3.1.1.13.2.1"/>
					<code>
						<xsl:attribute name="code" namespace="" select="'10154-3'"/>
						<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.6.1'"/>
						<xsl:attribute name="displayName" namespace="" select="'CHIEF COMPLAINT'"/>
						<xsl:attribute name="codeSystemName" namespace="" select="'LOINC'"/>
					</code>
					<title>
						<xsl:sequence select="'CHIEF COMPLAINT'"/>
					</title>

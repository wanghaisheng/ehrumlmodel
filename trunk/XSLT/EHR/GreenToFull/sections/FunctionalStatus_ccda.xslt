...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="functionalStatusCCDA">
        <xsl:param name="functionalStatus" />
		...
		<xsl:for-each select="$functionalStatus">
			<component>
				<section>
					...
					<code>
						<xsl:attribute name="code" namespace="" select="'47420-5'"/>
						<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.6.1'"/>
						<!-- <xsl:attribute name="displayName" namespace="" select="'Functional Status'"/>
						<xsl:attribute name="codeSystemName" namespace="" select="'LOINC'"/> -->
					</code>
					<title>
						<xsl:sequence select="'Functional Status'"/>
					</title>

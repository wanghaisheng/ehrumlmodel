...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="hospitalConsultationsCCDA">
        <xsl:param name="hospitalConsultations" />
		
		<xsl:for-each select="$hospitalConsultations">
			<component>
				<section>
					<templateId root="2.16.840.1.113883.10.20.22.2.42"/>
					<code>
						<xsl:attribute name="code" namespace="" select="'18841-7'"/>
						<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.6.1'"/>
						<xsl:attribute name="displayName" namespace="" select="'Hospital Consultations Section'"/>
						<xsl:attribute name="codeSystemName" namespace="" select="'LOINC'"/>
					</code>
					<title>
						<xsl:sequence select="'Hospital Consultations'"/>
					</title>

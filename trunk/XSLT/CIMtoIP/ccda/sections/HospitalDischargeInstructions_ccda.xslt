...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="hospitalDischargeInstructionsCCDA">
        <xsl:param name="hospitalDischargeInstructions" />
		
		<xsl:for-each select="$hospitalDischargeInstructions">
			<component>
				<section>
					<templateId root="2.16.840.1.113883.10.20.22.2.41"/>
					<code>
						<xsl:attribute name="code" namespace="" select="'8653-8'"/>
						<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.6.1'"/>
						<xsl:attribute name="displayName" namespace="" select="'HOSPITAL DISCHARGE INSTRUCTIONS'"/>
						<xsl:attribute name="codeSystemName" namespace="" select="'LOINC'"/>
					</code>
					<title>
						<xsl:sequence select="'Hospital Discharge Instructions'"/>
					</title>

...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="historyofPastIllnessCCDA">
        <xsl:param name="historyofPastIllness" />
		
		<xsl:for-each select="$historyofPastIllness">
			<component>
				<section>
					<templateId root="2.16.840.1.113883.10.20.22.2.20"/>
					<code>
						<xsl:attribute name="code" namespace="" select="'11348-0'"/>
						<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.6.1'"/>
						<xsl:attribute name="displayName" namespace="" select="'HISTORY OF PAST ILLNESS'"/>
						<xsl:attribute name="codeSystemName" namespace="" select="'LOINC'"/>
					</code>
					<title>
						<xsl:sequence select="'Past Medical History'"/>
					</title>

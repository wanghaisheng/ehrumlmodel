...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="hospitalDischargeStudiesSummaryCCDA">
        <xsl:param name="hospitalDischargeStudiesSummary" />
		
		<xsl:for-each select="$hospitalDischargeStudiesSummary">
			<component>
				<section>
					<templateId root="2.16.840.1.113883.10.20.22.2.16"/>
					<code>
						<xsl:attribute name="code" namespace="" select="'11493-4'"/>
						<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.6.1'"/>
						<xsl:attribute name="displayName" namespace="" select="'HOSPITAL DISCHARGE STUDIES SUMMARY'"/>
						<xsl:attribute name="codeSystemName" namespace="" select="'LOINC'"/>
					</code>
					<title>
						<xsl:sequence select="'Hospital Discharge Studies Summary'"/>
					</title>

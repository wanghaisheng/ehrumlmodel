...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="historyofPresentIllnessCCDA">
        <xsl:param name="historyofPresentIllness" />
		
		<xsl:for-each select="$historyofPresentIllness">
			<component>
				<section>
					<templateId root="1.3.6.1.4.1.19376.1.5.3.1.3.4"/>
					<code>
						<xsl:attribute name="code" namespace="" select="'10164-2'"/>
						<xsl:attribute name="codeSystem" namespace="" select="'2.16.840.1.113883.6.1'"/>
						<xsl:attribute name="displayName" namespace="" select="'HISTORY OF PRESENT ILLNESS'"/>
						<xsl:attribute name="codeSystemName" namespace="" select="'LOINC'"/>
					</code>
					<title>
						<xsl:sequence select="'History of Present Illness'"/>
					</title>

...
<xsl:stylesheet version="2.0"
		...
		<ClinicalDocument xmlns="urn:hl7-org:v3"
			...
			<xsl:call-template name="headerInformationCCDA">
				...
				<xsl:with-param name="documentTemplateId"
					select="'2.16.840.1.113883.3.1275.1.1.4.1'" />
				<xsl:with-param name="mnemonic" select="$mnemonic" />
			</xsl:call-template>

			<component>
				<xsl:choose>
					<xsl:when test="$mnemonic=$TOCConsultationRequest_Level1">
						<xsl:call-template name="CIMtoCCDA_TOC_CR_L1">
							<xsl:with-param name="tocSummary" select="toc:ToCSummary" />
							<xsl:with-param name="mnemonic" select="$mnemonic" />
						</xsl:call-template>
					</xsl:when>
					<xsl:when test="$mnemonic=$TOCConsultationRequest_Level2">
						<xsl:call-template name="CIMtoCCDA_TOC_CR_L2">
							<xsl:with-param name="tocSummary" select="toc:ToCSummary" />
							<xsl:with-param name="mnemonic" select="$mnemonic" />
						</xsl:call-template>
					</xsl:when>
					<xsl:when test="$mnemonic=$TOCConsultationRequest_Level3">
						<xsl:call-template name="CIMtoCCDA_TOC_CR_L3">
							<xsl:with-param name="tocSummary" select="toc:ToCSummary" />
							<xsl:with-param name="mnemonic" select="$mnemonic" />
						</xsl:call-template>
					</xsl:when>
	...
	<xsl:template name="CIMtoCCDA_TOC_CR_L1">
	...
	<xsl:template name="CIMtoCCDA_TOC_CR_L2">
		<xsl:param name="tocSummary" />
		<xsl:param name="mnemonic" />
		<xsl:call-template name="CIMtoCCDA_TOC_CR_L3">
	...
	<xsl:template name="CIMtoCCDA_TOC_CR_L3">
		<xsl:param name="tocSummary" />
		<xsl:param name="mnemonic" />
		<structuredBody>
		   ...
		   <!-- Physical Exam -->
		    <xsl:if test="fn:exists(toc:ToCSummary/toc:Body/toc:PhysicalExamination)">
		       <xsl:call-template name="physicalExaminationCCDA">
		           <xsl:with-param name="physicalExamination" select="$tocSummary/toc:Body/toc:PhysicalExamination" />
		           <xsl:with-param name="mnemonic" select="$mnemonic" /> 
		       </xsl:call-template>
	       </xsl:if>
		   ...
		   <!-- General Status -->
		    <xsl:if test="fn:exists(toc:ToCSummary/toc:Body/toc:GeneralStatus)">
		       <xsl:call-template name="generalStatusCCDA">
		           <xsl:with-param name="generalStatus" select="$tocSummary/toc:Body/toc:GeneralStatus" />
		           <xsl:with-param name="mnemonic" select="$mnemonic" /> 
		       </xsl:call-template>
		   </xsl:if>
		   		   
		   <!-- History of Past Illness -->
		   <xsl:if test="fn:exists(toc:ToCSummary/toc:Body/toc:HistoryofPastIllness)">
		       <xsl:call-template name="historyofPastIllnessCCDA">
		           <xsl:with-param name="historyofPastIllness" select="$tocSummary/toc:Body/toc:HistoryofPastIllness" />
		       </xsl:call-template>
		   </xsl:if>

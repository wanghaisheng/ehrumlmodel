...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="pocHealthCareServiceSectionCCDA">
        <xsl:param name="pocHealthCareServiceSection"/>
			<Service>
				<xsl:for-each select="$pocHealthCareServiceSection/@moodCode"> 
					<ServiceType> 
						<xsl:sequence select="fn:string(.)"/>
					</ServiceType> 
				</xsl:for-each>
						
                <xsl:for-each select="$pocHealthCareServiceSection/cda:id">
                ...
                <xsl:for-each select="$pocHealthCareServiceSection/cda:code">

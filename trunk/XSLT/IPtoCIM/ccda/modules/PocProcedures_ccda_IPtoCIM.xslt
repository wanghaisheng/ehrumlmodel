...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="pocProceduresSectionCCDA">
        <xsl:param name="pocProceduresSection"/>

			<Procedure>
				<xsl:for-each select="$pocProceduresSection/@moodCode"> 
					<ServiceType> 
						<xsl:sequence select="fn:string(.)"/>
					</ServiceType> 
				</xsl:for-each>
                <xsl:for-each select="$pocProceduresSection/cda:id">

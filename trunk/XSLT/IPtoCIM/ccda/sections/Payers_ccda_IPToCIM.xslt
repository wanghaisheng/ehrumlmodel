<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="payersSectionCCDA">
        <xsl:param name="payersSection"/>
        <xsl:param name="srcMnemonic"/>
			<Payers>				
              <xsl:for-each select="$payersSection/cda:text"> 
                  <Text> 
					<xsl:apply-templates select="$payersSection/cda:text/node()"/>
               	  </Text> 
              </xsl:for-each>
			</Payers>
    </xsl:template>        

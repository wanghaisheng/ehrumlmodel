...
<xsl:stylesheet version="2.0"
   ...
   <xsl:template match="cda:text//child::*">
    ...
    <xsl:template name="valueCD">
        ...
        <xsl:param name="elementName" />
            ...
            <xsl:for-each select="$value/@displayName">
                ...
                <xsl:for-each select="$value/cda:originalText/cda:reference">
	            <gcda:originalText>
    ...
    <xsl:template name="sxcmTS">
        ...
        <xsl:param name="elementName" />
            <xsl:element name="{$elementName}">
                <xsl:for-each select="$sxcmTS/@nullFlavor">
                    <xsl:attribute name="nullFlavor" select="fn:string(.)"/>
                </xsl:for-each>
                <xsl:for-each select="$sxcmTS/@value">
                    <xsl:attribute name="value" select="fn:string(.)"/>
                </xsl:for-each>
                <xsl:for-each select="$sxcmTS/@operator">
    ...
    <xsl:template name="personName">
        ...
        <xsl:element name="{$elementName}">
            ...
            <xsl:choose>
                <xsl:when test="fn:exists($personName/cda:family) or fn:exists($personName/cda:given) or fn:exists($personName/cda:prefix) or fn:exists($personName/cda:suffix)">
                    ...
                    <xsl:if test="fn:exists($personName/cda:validTime)">
                        <xsl:call-template name="ivlTS">
                            <xsl:with-param name="ivlTS" select="$personName/cda:validTime" as="node()" />
                            <xsl:with-param name="elementName" select="'gcda:validTime'" />
                        </xsl:call-template>
                    </xsl:if>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:for-each select="$personName/node()[./self::text()]">
                        <xsl:sequence select="fn:string(.)"/>
                    </xsl:for-each>
                </xsl:otherwise>
            </xsl:choose>
    ...
    <xsl:template name="organizationName">
        ...
        <xsl:element name="{$elementName}">
            ...
            <xsl:choose>
	            <xsl:when test="fn:exists($organizationName/cda:prefix) or fn:exists($organizationName/cda:suffix)">
                    ...
                    <xsl:if test="fn:exists($organizationName/cda:validTime)">
                        <xsl:call-template name="ivlTS">
                            <xsl:with-param name="ivlTS" select="$organizationName/cda:validTime" as="node()" />
                            <xsl:with-param name="elementName" select="'gcda:validTime'" />
                        </xsl:call-template>
                    </xsl:if>
		        </xsl:when>
		        <xsl:otherwise>
		            <xsl:for-each select="$organizationName/node()[./self::text()]">
		                <xsl:sequence select="fn:string(.)"/>
		            </xsl:for-each>
	            </xsl:otherwise>
            </xsl:choose>
    ...
    <xsl:template name="telecom">
        ...
        <xsl:param name="elementName" />
            <xsl:element name="{$elementName}">
		         ...
		         <xsl:for-each select="$telecom/cda:useablePeriod">
		             <xsl:call-template name="sxcmTS">
		                 <xsl:with-param name="sxcmTS" select="." as="node()" />
		                 <xsl:with-param name="elementName" select="'gcda:useablePeriod'" />
		             </xsl:call-template>
		         </xsl:for-each>
             </xsl:element>
    </xsl:template>
    <xsl:template name="address">
        ...
        <xsl:param name="elementName" />
            <xsl:element name="{$elementName}">
                 ...
                 <xsl:for-each select="$address/cda:useablePeriod">
                     <xsl:call-template name="sxcmTS">
                         <xsl:with-param name="sxcmTS" select="." as="node()" />
                         <xsl:with-param name="elementName" select="'gcda:validTime'" />
                     </xsl:call-template>
